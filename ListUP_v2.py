...
##################### Make arrays from the string data ####################
...
#core=list((set(wholestr).symmetric_difference(set(capstr))))
...
data1.insert(4, "E", 2)
data1.insert(5, "F", 2)
data1.to_csv('cap.csv', index =int(1))

######################## modification of core data ############################
data3.insert(3, "D", 1)
data3.insert(4, "E", 1)
data3.insert(5, "F", 1)
data3.to_csv('core.csv', index =int(1))

##################### writing the whole structure file ########################
f3=pd.read_csv("cap.csv")
f4=pd.read_csv("core.csv")
data4=pd.concat([f3, f4])
data4.to_csv('shape.csv', index = False)

with open ('shape.csv', 'r') as finaldata, open ('shape1.dat', 'w')as finalfile:
    content=finaldata.read().replace(',',' ')
    finalfile.write(content)
    
with open('shape1.dat') as fin, open('shape.dat','w') as fout:
  for line in fin:
    fout.write( "\t".join(line.split()) + "\n" )
    
###################### removal of files ########################
os.remove('cap_refined.txt')
os.remove('core_ref.txt')
os.remove('core_refined.txt'),
os.remove('testcap.csv')
os.remove('testcore.csv')
os.remove('whole.txt')
os.remove('cap.txt')
os.remove('core.txt')
os.remove('shape1.dat')
